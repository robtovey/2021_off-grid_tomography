...
class tomo_iter_alg(tomo_alg):
    ...
    def run(self, maxiter=100, callback=None, callback_freq=10, **kwargs):
        ...
        if callback is None:
            ...
            self.step(0, maxiter)
            print('finished after ' + _timelen(time() - tic))
        elif callback == 'silent':
            self.step(0, maxiter)
        else:
        ...
        if callback in (None, 'silent'):
            return self.getRecon()
        else:
            return self.getRecon(), Q

    def start(self, **_init): raise NotImplementedError
    def step(self, i, niter): raise NotImplementedError
    def callback(self, names): raise NotImplementedError


class tomo_iter_alg_lazy(tomo_iter_alg):
    '''
    An iterable algorithm but where the iteration is performed by a third-party function
    '''
    ...
    def run(self, maxiter=100, callback=None, callback_freq=10, **kwargs):
        ...
        if callback is None:
            ...
            self.step(0, maxiter, lambda x: self.updateRecon(x))
        else:
            callback = ('Iter', 'Time',) + Tuple(callback)
            prints = []
            ...
            i = [0, min(callback_freq, maxiter)]
            print(padstr(callback[0], 6), padstr(callback[1], 7),
                  *(padstr(c) for c in callback[2:]))
            prints.append((i[0], time() - tic)
                           +Tuple(self.callback(callback[2:])))
            print(padstr('%3d%%' % (i[0] / maxiter * 100), 6), padstr(_timelen(prints[-1][1]), 7),
                  *(frmt(c) for c in prints[-1][2:]), flush=True)

            self._early_stop = False  # flag for early stopping
            def _callback(recon, just_print=False):
                if not just_print:
                    self.updateRecon(recon)
                if i[0] == i[1]:
                    i[1] = min(i[0] + callback_freq, maxiter)
                    prints.append((i[0], time() - tic,) +
                                  Tuple(self.callback(callback[2:])))
                    print(padstr('%3d%%' % (i[0] / maxiter * 100), 6), padstr(_timelen(prints[-1][1]), 7),
                          *(frmt(c) for c in prints[-1][2:]), flush=True)
            ...
            self.step(0, maxiter, _callback)
            if i[0] < i[1]:
                i[1] = i[0]
                _callback(None, just_print=True)
            print()
    ...
    def updateRecon(self, recon): self.recon = recon  # current estimate
...
class f2op(LinearOperator):
    def __init__(self, fwrd, bwrd, shape0, shape1, dtype='f4'):
        LinearOperator.__init__(self, dtype, (np.prod(shape1), np.prod(shape0)))
        self.fwrd, self.bwrd, self._shape0, self._shape1 = fwrd, bwrd, shape0, shape1

    def __call__(self, x): return self.fwrd(x.reshape(self._shape0))

    def _matvec(self, x): return self.fwrd(x.reshape(self._shape0)).ravel()
    def _rmatvec(self, y): return self.bwrd(y.reshape(self._shape1)).ravel()
...
class L2fidelity(smoothFunc, proxFunc):
    def __init__(self, scale=1, A=None, translation=None, normA=None, tol=1e-10):
        '''
        f(x) = scale/2|Ax-translation|^2
             = s/2|Ax-d|^2
        df = s A^*(Ax-d)
        lip = s |A|^2
        prox = argmin 1/2|x-X|^2 + st/2|Ax-d|^2
              = (id+st A^*A)^{-1}(X+st A^*d)
        ...
        if translation is None:
            ...
            def setproxf(t): self.__proxparam = self.scale * t
            def proxf(x): return CG_LSQRS(
                        A=lambda X: X + self.A.T.dot(self.__proxparam * self.A.dot(X)),
                        b=x, x0=x, maxiters=min(100, min(self.A.shape)), tol=tol * self.__proxparam
                    )
        else:
...
#                 return tmp if self.scale == 1 else self.scale * tmp
            ...
            def f(x): return (self.scale * .5) * ((A.dot(x) - translation) ** 2).sum()
            def df(x):
                tmp = A.T.dot(A.dot(x) - translation)
                return tmp if self.scale == 1 else self.scale * tmp

            def setproxf(t): self._proxparam = scale * t, (scale * t) * Ad
            def proxf(x): return CG_LSQRS(
                        A=lambda X: X + self.A.T.dot(self._proxparam[0] * self.A.dot(X)),
                        b=x + self._proxparam[1], x0=x, maxiters=min(100, min(self.A.shape)), tol=tol  # * self.__proxparam[0]
                    )
        ...
        self.scale, self.A, self._normA, self.tol = scale, A, normA, tol

    @property
    def Lip(self): return self.scale * self._normA ** 2


def CG_LSQRS(A, b, x0, maxiters=1000, tol=1e-16):
    x = x0.copy()
    r = b - A(x)
    En = vecNorm(r) ** 2
    tol = max(En, 1) * tol

    if En <= tol:  # trivial case starting at a solution
        return x

    for n in range(maxiters):
        if n % 100 == 0:  # reset for stability
            p = r.copy()
        ...
        if n % 20 == 0:  # recompute r exactly every 20 iterations
            r = b - A(x)
        else:
            r -= alpha * Ap
...
# np.random.seed(1)
#
# A = np.random.rand(200, 100); A2 = A.T.dot(A)
# x = np.random.rand(100)
# b = np.random.rand(200)
# y = CG_LSQRS(lambda X:A2.dot(X), A2.dot(x), 0 * x, tol=1e-10)
# print('\nFinal error:', abs(A.dot(x - y)).max(), '\n')
# # y = CG_LSQRS(lambda X:A2.dot(X), A.T.dot(b), x, tol=1e-10)
# # print('\nFinal error:', abs(A.dot(y) - b).max())
# exit()
...
class CONSTRAINT(proxFunc):
    def __init__(self, translation=None):
        '''
        Let d = translation
        f(x) = 0 if (x == d) else infinity
        df(x) = 0
        lip = hess = nan
        prox(x,t) = d

        g(y) = sup_x <y,x> s.t. x = d
             = <y,d>
        dg(y) = d
        lip = hess = 0
        prox(y,t) = argmin 1/2|Y-y|^2 + t<Y,d>
                  = y-td
        ...
        if translation is None:
            ...
            def proxf(x): return 0 * x
            def proxg(y): return y

            def violation(x): return norm(x, np.inf)
        else:
            ...
            def proxf(_): return translation
            def proxg(y): return y - self.__proxgparam

            def violation(x): return norm(x - translation, np.inf)

        proxFunc.__init__(self, f, prox=proxf, setprox=setproxf, violation=violation)
        dual = proxFunc(g, prox=proxg, setprox=setproxg)
        dualableFunc(self, dual)


class L2(proxFunc, smoothFunc):
    def __init__(self, scale=1, translation=None):
        ...
        proxFunc.__init__(self, f, prox=proxf, setprox=setproxf, grad=df)
        smoothFunc.__init__(self, f, grad=df, Lip=scale)
...
#                         shrink1D(ww, self.__proxfparam, out=ww)  # inplace edit of w
    ...
    def listify(self, w):
        ...
        return sum((list(ww.values()) for ww in w[1:]), [w[0]])
...
class PDHG(tomo_iter_alg):
    ...
    def __init__(self, A, f, g, op=None):
        ...
        self.s, self.t, self.theta = None, None, None
        self.tol = None
    ...
    def setParams(self, A=None, f=None, g=None, x=None, y=None, sigma=None,
        ...
        if (sigma is not None) and (tau is not None):
            ...
            if steps[0].lower() == 'b':
                normA /= 3
            sigma, tau = 1 / (balance * normA), balance / normA
        ...
        self.theta = 1 if self.theta is None else self.theta
        if steps in [None, 'None', 'none']:
            self._stepsize = lambda _: False
        elif steps[0].lower() == 'a':  # adaptive balance
        ...
        elif steps[0].lower() == 'b':  # backtracking norm estimate
            if not (np.isscalar(self.s) and np.isscalar(self.t)):
                raise ValueError(
            ...
            if not (np.isscalar(self.s) and np.isscalar(self.t)):
                raise ValueError(
                    'Step sizes must be scalar for strong stepsize choice')
            self._stepsize = self._strongstep(normA=normA, **stepParams)
        else:
            raise ValueError('steps must be None, adaptive, backtrack, or strong.')
    ...
    def step(self, i, niter):
        for _ in range(niter):
            # Primal step:
            tmp = self.x
            self.x = self.f.prox(self.x - self.s * self.Ay)
#             if self.theta == 1:
#                 self.x = self.f.prox(self.x - self.s * (2 * self.Ay - self.Aym1))
#             else:
#                 self.x = self.f.prox(self.x - self.s * ((1 + self.theta) * self.Ay - self.theta * self.Aym1))
            ...
            tmp = self.y
            if self.theta == 1:
                self.y = self.g.prox(self.y + self.t * (2 * self.Ax - self.Axm1))
            else:
                self.y = self.g.prox(self.y + self.t * ((1 + self.theta) * self.Ax - self.theta * self.Axm1))
#             self.y = self.g.prox(self.y + self.t * self.Ax)
    ...
    def _backtrack(self, beta=0.95, gamma=0.8):
        ...
        def stepsize(alg):
            ...
            b = (2 * alg.t * alg.s * vecIP(dx, alg.Ay - alg.Aym1).real) / (
                alg.s * vecNorm(dx) ** 2 + alg.t * vecNorm(dy) ** 2 + 1e-10)
    ...
    def _adaptive(self, alpha=0.5, eta=0.95, delta=1.5, s=1):
        ...
        def stepsize(alg):
            p = vecNorm((alg.x - alg.xm1) / alg.s - alg.Ay + alg.Aym1)
            d = vecNorm((alg.y - alg.ym1) / alg.t + alg.Ax - alg.Axm1)
    ...
    def _strongstep(self, f=0, g=0, normA=None, L=None):
        if normA is None:
            normA = 1 / (self.s * self.t) ** .5
        if L is None:
            assert hasattr(self.f, 'Lip')
            L = self.f.Lip
        self.stepParams = {'f': f, 'g': g, 'A':normA, 'L':L}

        self.s = (L * (g + f * normA ** 2)) ** .5 / normA
        self.t = 1 / (self.s * normA ** 2)
        self.theta = max((1 - f * normA ** 2 / (L ** 2 * (f + 2 * self.s))) / (1 + self.t * g),
                         2 / (2 + self.s / L))
    ...
    @property
    def _grad(self):
        p = vecNorm((self.x - self.xm1) / self.s - self.Ay + self.Aym1)
        d = vecNorm((self.y - self.ym1) / self.t + self.Ax - self.Axm1)
        return p / (1e-8 + vecNorm(self.x)) + d / (1e-8 + vecNorm(self.y))

    @property
    def _prim(self): return self.f(self.x) + self.g.dual(self.Ax)
    @property
    def _dual(self): return self.f.dual(-self.Ay) + self.g(self.y)
...
class SplitBregman(tomo_iter_alg):
    '''
    Implementation of the accelerated ADMM method as proposed in
    @article{goldstein2014fast,
     ...
     '''
    def __init__(self, A, f, g, z=None, op=None):
        '''
        Minimise f(x) + g(y)
        such that Ax = y + z
        '''
        ...
        self.A = A
        self.f, self.g = f, g
        self.x, self.y, self.z = None, None, z
        self.t, self.tol = None, None
        self._start_step = None

    def setParams(self, A=None, f=None, g=None, x=None, y=None, z=None, tau=None,
                  eta=None, eps=None, tol=None, steps=None, **_):
        # Reset problem
        if f is not None:
            self.f = f
        if g is not None:
        ...
        if x is not None:
            ...
            self.x = self.xm1 = getVec(self.A, rand=False)
        if y is not None:
            self.y = y
        elif self.y is None:
            self.y = getVec(self.A.T, rand=False)  # = 0
        if z is not None:
            self.z = z
        elif self.z is None:
            self.z = getVec(self.A.T, rand=False)  # = 0

        self._L = getVec(self.A.T, rand=False)  # = 0
        self.Ax = self.A * self.x

        # Set initial step sizes
        if tau is None:
            tau = 1 if self.t is None else self.t
        if eta is None:
            eta = 0.999
        if eps is None:
            eps = 0.2
        if steps is None:
            if self._start_step is None:
                self._start_step = self._classic_scalar_step
                self._end_step = self._accel_restart
        elif steps[0].lower() == 'f':  # fast version
            self._start_step = self._classic_scalar_step
            self._end_step = self._accel_restart
        elif steps[0].lower() == 'a':  # adaptive version
            self._start_step = self._adapt_step
            self._end_step = None
        elif steps[0].lower() == 'b':  # both adaptive and fast
            self._start_step = self._adapt_step
            self._end_step = self._accel_restart
        self.t, self.eta, self.eps = tau, eta, eps
        self.f.setprox(self.t)  # TODO: implicit assumption that f knows about A
        self.g.setprox(self.t)
        self._a, self._c = 1, np.inf
    ...
    def step(self, i, niter):
        for _ in range(niter):
            x, y, L, Ax, c = self._start_step()

            if self._end_step is None:
                self.x, self.xm1, self.y, self._L, self._c = x, self.x, y, L, c
            else:
                self._end_step(x, y, L, Ax, c)

    def _classic_step(self):  # step with sequence of tau
        z = self.z + (1 / self.t) * self._L
        x = self.f.prox(z + self.y)  # x = argmin f(x) + t/2|Ax-(...)|^2
        Ax = self.A * x
        y = self.g.prox(Ax - z)  # y = argmin g(y) + t/2|y-(...)|^2
        L = self._L + self.t * (self.z + y - Ax)
        c = vecNorm(L - self._L) ** 2 / self.t ** 2 + vecNorm(y - self.y) ** 2
        return x, y, L, Ax, c
    ...
    def _adapt_step(self):
        ...
        Ax = self.A * x
        y = self.g.prox(Ax - z)  # y = argmin g(y) + t/2|y-(...)|^2
        L = self._L + self.t * (self.z + self.y - Ax)

        DL = L - self._L
        DH = Ax - self.Ax
        DY = y - self.y
        DLDL, DHDL, DHDH = vecIP(DL, DL), vecIP(DH, DL), vecIP(DH, DH)
        DYDY, DYDL = vecIP(DY, DY), vecIP(DY, DL)
        c = DLDL / self.t ** 2 + DYDY ** 2

        out = x, y, self._L + DL, Ax, c

        if DLDL * DHDH * DYDY == 0:
#             self.t /= 2  # if DY=0 then constraint term is too tight
#             self.f.setprox(self.t); self.g.setprox(self.t)
            return out

        a_SD, a_MG = DLDL / DHDL, DHDL / DHDH
        b_SD, b_MG = DLDL / DYDL, DYDL / DYDY

        a = a_MG if 2 * a_MG > a_SD else a_SD - .5 * a_MG
        b = b_MG if 2 * b_MG > b_SD else b_SD - .5 * b_MG
        a, b = a_SD, b_SD
        a_C, b_C = DHDL / (DHDH * DLDL) ** .5, DYDL / (DYDY * DLDL) ** .5

        eps = self.eps
        if a_C > eps and b_C > eps:
            self.t = (a * b) ** .5
        elif a_C > eps:
            self.t = a
        elif b_C > eps:
    ...
    def _classic_scalar_step(self):  # step with constant tau
        z = self.z + self._L
        x = self.f.prox(z + self.y)  # x = argmin f(x) + t/2|Ax-(...)|^2
        Ax = self.A * x
        y = self.g.prox(Ax - z)  # y = argmin g(y) + t/2|y-(...)|^2
        L = self._L + self.z + y - Ax
        c = vecNorm(L - self._L) ** 2 + vecNorm(y - self.y) ** 2
        return x, y, L, Ax, c

    def _accel_restart(self, x, y, L, Ax, c):
        if c < self.eta * self._c:
#             self.__buf = y, L
            self.x, self.xm1, self.Ax = x, self.x, Ax
            a = .5 * (1 + np.sqrt(1 + 4 * self._a ** 2))
            r = (self._a - 1) / a
            self.y, self._L = y + r * (y - self.y), L + r * (L - self._L)
            self._c, self._a = c, a
        else:
#             self.y, self._L = self.__buf
            self.y, self._L = self.y, self._L  # NOTE: this is not quite the original algorithm
            self._c, self._a = self._c / self.eta, 1

    def _strong_stepsize(self, f, g, A): return (f * g ** 2 / A ** 2) ** (1 / 3)
    ...
    @property
    def _prim(self):
        return self.f(self.x) + self.g(self.Ax - self.z)

    @property
    def _saddle(self): return self._prim + .5 * self.t * vecNorm(self.z + self.y - self.Ax) ** 2

    @property
    def _gap(self): return self._c ** .5
    ...
    @property
    def _violation(self):
        return (self.f.violation(self.x) + self.g.violation(self.y)
                +vecNorm(self.z + self.y - self.Ax))
...
class FISTA(tomo_iter_alg):
    ...
    def __init__(self, f, g, op=None):
        ...
        self._steps = 'Greedy'  # default step-size choice
    ...
    def setParams(self, f=None, g=None, x=None, tol=None,
        ...
        self.t = np.divide(1, self.f.Lip, out=np.zeros_like(self.f.Lip), where=self.f.Lip != 0)
        self.g.setprox(self.t)
...
#             Check for restarting:
            ...
            if self._stepsize(self):
                self.g.setprox(self.t)
            elif _ % 10 == 9:  # check periodically for convergence
                self.__converged = (self._step <= self.tol)
...
class TV_prox(proxFunc):
    def __init__(self, size, D, scale=1, normD=None, lazypos=False, maxiter=10000, tol=1e-5):
        '''
        f(x) = scale||Dx|_2|_1 
        df(x) = scale * D^*(Dx/|Dx|_2)
        lip = hess = nan
        prox(x,t) = argmin_X 1/2|X-x|^2 + scale*t|DX|_2
                = D^* argmin_{|Y|< scale*t} 1/2|D^*Y|_2^2 - <Dx,Y>
                
        The primal-dual gap is 1/2|X-x|^2 + <DX,Y>

        lazypos is a flag to impose non-negativity on x. For the prox this just replaces x with max(x,0)
        '''

        def f(x): return scale * norm((self.D * x.ravel()).reshape(size, -1), 2, 1).sum()

        def setproxf(t): self.__proxfparam = t, scale * t

        def proxf(x):
            if lazypos:
                x = np.maximum(0, x)
            self.__proxDX = self.D * x
            Y = self._proxalg.run(maxiter=maxiter, tol=tol, callback='silent')
#             Y = self._proxalg.run(maxiter=maxiter, callback_freq=maxiter // 10, tol=tol, callback=('primal', 'step'))[0]
            return x - self.D.T * Y
        ...
        proxFunc.__init__(self, f, prox=proxf, setprox=setproxf, violation=violation)
        self.shape, self.scale, self.D = (size, -1), scale, D
        if normD is not None:
            self.normD = normD
        elif hasattr(D, 'norm'):
            self.normD = D.norm()
        else:
            self.normD = 1.01 * poweriter(D)

        def g(y):
            Dy = self.D.T * y
            return .5 * Dy.dot(Dy) - self.__proxDX.dot(y)

        def dg(y): return self.D * (self.D.T * y) - self.__proxDX

        def proxh(y): return project(y.reshape(self.shape), self.__proxfparam[1])

        self._proxalg = FISTA(
                f=smoothFunc(g, dg, self.normD ** 2),
                g=proxFunc(lambda x:0, prox=proxh, setprox=lambda t: None),
                op=D.T,
            )
        self._proxalg.setParams(steps='Greedy', restarting=True)
...
class TV_base:
    ...
    def setParams(self, data, op=None, x=None, y=None, **kwargs):
        ...
        self.data = data  # before re-scaling

        # rescale data such that w=1 corresponds to an over-regularised solution
        dataScale = max(abs(R.T * data.ravel()).max(), 1e-6)
        data = data.__array__() / dataScale
        return R, normR, data, dataScale
    ...
    def start(self, data=None, **kwargs):
        ...
        self.setParams(data=data, **kwargs)

    def getRecon(self): return self.x.reshape(self.d.vol_shape) * self.dataScale


class TV_PDHG(PDHG, TV_base):
    def __init__(self, shape, pos=True, op=None, **kwargs):
        '''
        Minimise the energy:
        1/2|op(u)-data|^2 + weight*|\nabla^{order}u|_1
        where:
        ...
        PDHG.__init__(self, None,
                      ...
                      None, op=op)
        TV_base.__init__(self, shape, pos=pos, **kwargs)

    def setParams(self, data, x=None, y=None, **kwargs):
        R, normR, data, dataScale = TV_base.setParams(self, data, **kwargs)
        ...
        rat = normD / normR
        data = data / rat  # normalisation from operator
        self.dataScale = dataScale * rat ** 2
        ...
        if self.weight > 1e-8:
            gstar = stackProxFunc(
                ...
                L1_2(size=np.prod(self.d.vol_shape), scale=self.weight)
            )
        else:
            gstar = stackProxFunc(
                CONSTRAINT(translation=data),
                L1_2(size=np.prod(self.d.vol_shape))
            )
    ...
    @property
    def _prim(self): return self.__Escale * (self.f(self.x) + self.g.dual(self.Ax))
    @property
    def _dual(self): return self.__Escale * (self.f.dual(-self.Ay) + self.g(self.y))
    def start(self, **kwargs): TV_base.start(self, **kwargs)
    def getRecon(self): return self.x[0].reshape(self.d.vol_shape) * self.dataScale


class TV_SplitBregman(SplitBregman, TV_base):
    def __init__(self, shape, op=None, **kwargs):
        '''
        Minimise the energy:
        1/2|op(u)-data|^2 + weight*|\nabla^{order}u|_1
        where:
            ...
            pos=True: If true then a non-negativity constraint is applied
        '''
        SplitBregman.__init__(self, None, None, None, op=op)
        TV_base.__init__(self, shape, **kwargs)
    ...
    def setParams(self, data, x=None, y=None, **kwargs):
        ...
        if x is None:
            ...
            x = x.ravel() / self.dataScale

        f = L2fidelity(scale=1, A=R, normA=normR, translation=data)

        if self.weight > 0:
            def prox(x):
                return CG_LSQRS(
                        A=lambda X: self.d.T * (self.d * X) + R.T.dot(f._proxparam[0] * R.dot(X)),
                        b=self.d.T * x + f._proxparam[1], x0=self.x, maxiters=min(100, min(R.shape)), tol=f.tol
                    )
            f._prox = prox

            SplitBregman.setParams(self, A=self.d, f=f,
                           g=L1_2(size=np.prod(self.d.vol_shape), scale=self.weight), x=x, **kwargs)
        else:
            def prox(x):
                return CG_LSQRS(
                        A=lambda X: R.T.dot(R.dot(X)),
                        b=f._proxparam[1], x0=self.x, maxiters=min(100, min(R.shape)), tol=f.tol
                    )
            f._prox = prox

            SplitBregman.setParams(self, A=self.d, f=f,
                           g=L1_2(size=np.prod(self.d.vol_shape), scale=1), x=x, **kwargs)

    def start(self, **kwargs): TV_base.start(self, **kwargs)
    def getRecon(self): return self.x.reshape(self.d.vol_shape) * self.dataScale


class TV_FISTA(FISTA, TV_base):
    def __init__(self, shape, op=None, **kwargs):
        '''
        Minimise the energy:
        1/2|op(u)-data|^2 + weight*|\nabla^{order}u|_1
    ...
    def setParams(self, data, x=None, **kwargs):
        R, normR, data, dataScale = TV_base.setParams(self, data, **kwargs)
        self.dataScale = dataScale
...
#         normR = np.minimum(normR, (R.T * (R * (getVec(R) + 1))) ** .5)  # diagonal preconditioning

        FISTA.setParams(self,
                       f=L2fidelity(scale=1, A=R, normA=normR, translation=data), x=x, **kwargs,
                       g=TV_prox(np.prod(self.d.vol_shape), self.d, self.weight, lazypos=self.pos, maxiter=100, tol=1e-5))

    def start(self, **kwargs):
        TV_base.start(self, **kwargs)
        self.counter = 0

    def getRecon(self): return self.x.reshape(self.d.vol_shape) * self.dataScale


class SmoothTV(tomo_iter_alg_lazy, TV_base):
    def __init__(self, shape, pos=True, op=None, eps=1e-2, **kwargs):
        tomo_iter_alg_lazy.__init__(self, op=op)
        TV_base.__init__(self, shape, pos=pos, **kwargs)
    ...
    def setParams(self, data, x=None, **kwargs):
        ...
        # Already normalised such that max(R*Rx) = max(R*data) = 1
        # If R is diagonal then max(x) <= 1/|R|^2 is the scaling

        size, t = np.prod(self.d.vol_shape), (self.eps / self.dataScale) ** 2
        try:
            @numba.guvectorize(['void(f8[:],f8[:])'], '(n)->()', target='parallel', fastmath=True)
            def smoothL1(Dx, out):
                s = np.sqrt(Dx[0] ** 2 + Dx[1] ** 2 + t)
                out[0] = s
                Dx[0] /= s
                Dx[1] /= s
        except Exception:
            def smoothL1(Dx):
                s = np.sqrt((Dx * Dx).sum(1) + t)
                Dx /= s[:, None]
                return s

        def f_Df(x, grad=True):
            '''
            f(x) = 1/2|Rx-b|^2 + w*sum(sqrt(|Dx|^2+eps^2))
            
            Df(x) = R^*(Rx-b) + w D^* (Dx/sqrt(|Dx|^2+eps^2))
            '''

            res = R * x - data
            Dx = (self.d * x).reshape(size, -1)
#             T = np.sqrt((Dx * Dx).sum(1) + t); Dx /= T[:,None]
            T = smoothL1(Dx).reshape(size, 1)

            a, b = 1 / self.weight ** .5, self.weight ** .5

            f = .5 * a * np.linalg.norm(res, ord=2) ** 2 + b * T.sum()
            if grad:
                Df = R.T * (a * res) + b * self.d.T * Dx.ravel()
                return f, Df
            else:
                return f
        self.f_Df = f_Df
        ...
        if x is None:
            ...
            x = x / self.dataScale
        self.x = self.xm1 = x.ravel()

    def start(self, **kwargs): TV_base.start(self, **kwargs)

    def step(self, i, niter, callback):
        from scipy.optimize import minimize, Bounds
        minimize(self.f_Df, self.x, method='L-BFGS-B', jac=True,
                 bounds=(Bounds(0, np.inf, keep_feasible=True) if self.pos else None),
                 options={'maxcor':30, 'maxiter':niter - i + 1}, callback=callback)
    ...
    def callback(self, names):
        for n in names:
            if n == 'primal':
                ...
                yield self._step
    @property
    def _prim(self): return self.f_Df(self.x, grad=False)
    @property
    def _step(self): return vecNorm(self.x - self.xm1) / (1e-8 + vecNorm(self.x))
    def updateRecon(self, x): self.x, self.xm1 = x, self.x
    def getRecon(self): return self.x.reshape(self.d.vol_shape) * self.dataScale


def TV(*args, alg='PDHG', **kwargs):
    ALGS = {'PDHG':TV_PDHG, 'SplitBregman':TV_SplitBregman, 'FISTA':TV_FISTA}
    if alg in ALGS:
        return ALGS[alg](*args, **kwargs)
    else:
        raise NotImplementedError
...
class NonNeg_prox(proxFunc):
    def __init__(self, size, W, scale=1, maxiter=100, tol=1e-5):
        '''
        f(x) =  W(x) + contstraint(x >= 0)
        df(x) = dW
        lip = hess = nan
        prox_f(x,t) = argmin_{X>0} 1/2|X-x|^2 + t*scale*W(x)    -->    T = t*scale
                = argmin_{X>0} (1-a)/(2T)|X-x|^2 + a/(2T)|X-x|^2 + W(x)
        
        h(y) = a/(2T)|y-x|^2 + W(y)
        prox_h(y,s) = argmin_Y 1/2|Y-y|^2 + a*s/(2T)|Y-x|^2 + s*W(Y)
            = argmin_Y (1+a*s/T)/2|Y|^2 - <Y,y+a*s/T*x> + s*W(Y)
            = prox_W( (y+a*s/T*x)/(1+a*s/T), s/(1+a*s/T) )

        g(y) = (1-a)/(2T)|y-x|^2 + constraint(y >= 0)
        prox_g(y,s) = argmin_{Y>0} 1/2|Y-y|^2 + (1-a)*s/(2T)|Y-x|^2
            = max(0, (y + (1-a)*s/T*x)/(1+(1-a)*s/T))
        ...
        def setproxf(t):
            a, T = 2 / 3, self.scale * t
#             a, T = .5, self.scale * t
            self.__proxfT = (1 - a) / T, a / T, T

        def proxf(x):
            self.__proxfX = x
            self._proxalg.setParams(tau=self._proxalg._strong_stepsize(self.__proxfT[0],
                                                                       self.__proxfT[1], 1))
            return self._proxalg.run(maxiter=maxiter, tol=tol, callback='silent')
#             return self._proxalg.run(maxiter=maxiter, tol=tol, callback=('primal', 'step', 'gap'))[0]

        violation = lambda x: max(0, -x.min())

        proxFunc.__init__(self, f, prox=proxf, setprox=setproxf, violation=violation)
        self.size, self.scale, self.W = size, scale, W

        # non-negativity function
        def g(y): return .5 * self.__proxfT[0] * vecNorm(y - self.__proxfX) ** 2
        def setproxg(s): self.__proxgT = 1 / (1 + self.__proxfT[0] * s), self.__proxfT[0] * s * self.__proxfX
        def proxg(y): return np.maximum(0, self.__proxgT[0] * (y + self.__proxgT[1]))

        # W function
        def h(y): return .5 * self.__proxfT[1] * vecNorm(y - self.__proxfX) ** 2 + W(y)
        def setproxh(s):
            self.__proxhT = 1 / (1 + self.__proxfT[1] * s), self.__proxfT[1] * s * self.__proxfX
            W.setprox(s / (1 + self.__proxfT[1] * s))
        def proxh(y): return W.prox(self.__proxhT[0] * (y + self.__proxhT[1]))

        I = scalar_mat((size, size), scale=1)
        self._proxalg = SplitBregman(A=I, op=I,
                                     g=proxFunc(g, prox=proxg, setprox=setproxg, violation=violation),
                                     f=proxFunc(h, prox=proxh, setprox=setproxh))


class Wavelet_base:
    def __init__(self, shape, wavelet='haar', mode='constant', weight=0.1, pos=False):
        '''
        Minimise the energy:
        ...
        self.wavelet, self.mode, self.pos = wavelet, mode, pos
        self.data = None
    ...
    def setParams(self, data, op=None, x=None, **kwargs):
        ...
        self.pos = kwargs.get('pos', self.pos)
        if self.pos:
            g = NonNeg_prox(size=np.prod(self.vol_shape), scale=self.weight ** .5,
                            W=WaveletL1(wavelet=self.wavelet, mode=self.mode, vol_shape=self.vol_shape))
        else:
            g = WaveletL1(scale=self.weight ** .5, wavelet=self.wavelet, mode=self.mode, vol_shape=self.vol_shape)

        W = g.W.fwrd if self.pos else g.fwrd
        self.dataScale = max(max(abs(w).max() for w in W(R.T.dot(data.ravel()), listify=True)), 1e-6)
        self.data = data  # pre-scaled

        f = L2fidelity(scale=1 / self.weight ** .5, A=R, normA=normR, translation=data.__array__() / self.dataScale)
...
class Wavelet_FISTA(FISTA, Wavelet_base):
    def __init__(self, shape, op=None, **kwargs):
        FISTA.__init__(self, None, None, op=op)
        Wavelet_base.__init__(self, shape, **kwargs)

    def setParams(self, data, **kwargs):
        x, f, g, kwargs = Wavelet_base.setParams(self, data, **kwargs)

        FISTA.setParams(self, f=f, g=g, x=x, **kwargs)

    def start(self, **kwargs):
        Wavelet_base.start(self, **kwargs)
        self.counter = 0

    def getRecon(self): return Wavelet_base.getRecon(self)


class Wavelet_SplitBregman(SplitBregman, Wavelet_base):
    def __init__(self, shape, op=None, **kwargs):
        SplitBregman.__init__(self, A=scalar_mat((np.prod(shape),) * 2, scale=1), f=None, g=None, z=0, op=op)
        Wavelet_base.__init__(self, shape, **kwargs)

    def setParams(self, data, **kwargs):
        x, f, g, kwargs = Wavelet_base.setParams(self, data, **kwargs)
        SplitBregman.setParams(self, f=f, g=g, x=x, **kwargs)

    def start(self, tau=1e-4, **kwargs):
        Wavelet_base.start(self, tau=tau, **kwargs)
...
class SmoothWavelet(tomo_iter_alg_lazy, Wavelet_base):
    def __init__(self, shape, pos=False, op=None, eps=1e-3, **kwargs):
        tomo_iter_alg_lazy.__init__(self, op=op)
        Wavelet_base.__init__(self, shape, **kwargs)
        self.pos = pos
        self.eps = eps  # smoothing parameter
    ...
    def setParams(self, data, x=None, **kwargs):
        x, f, g, kwargs = Wavelet_base.setParams(self, data, **kwargs)
        data = self.data.__array__().ravel() / self.dataScale
        W = g.W if self.pos else g
        self.eps = kwargs.get('eps', self.eps)
        # Already normalised such that max(R*Rx) = max(R*data) = 1
        # If R is diagonal then max(x) <= 1/|R|^2 is the scaling
        ...
        def f_Df(x, grad=True):
            '''
            f(x) = 1/2|Rx-b|^2 + w*sum(sqrt(|Wx|^2+eps^2))
            
            Df(x) = R^*(Rx-b) + w W^* (Dx/sqrt(|Wx|^2+eps^2))
            '''

            res = f.A * x - data
            Wx = W.fwrd(x)
            Wx_list = W.listify(Wx)
            T = [np.sqrt((ww * ww) + self.eps ** 2 / f._normA ** 4) for ww in Wx_list]

            a, b = 1 / self.weight ** .5, self.weight ** .5

            F = .5 * a * np.linalg.norm(res, ord=2) ** 2 + b * sum(t.sum() for t in T)
            if grad:
                for i in range(len(T)):
                    Wx_list[i] /= T[i]
                DF = f.A.T * (a * res) + b * W.bwrd(Wx).ravel()
                return F, DF
            else:
                return F
        self.f_Df = f_Df
    ...
    def start(self, **kwargs): Wavelet_base.start(self, **kwargs)

    def step(self, i, niter, callback):
        from scipy.optimize import minimize, Bounds
        minimize(self.f_Df, self.x, method='L-BFGS-B', jac=True,
                 bounds=Bounds(0, np.inf) if self.pos else None,
                 options={'maxcor':30, 'maxiter':niter - i + 1}, callback=callback)
...
def Wavelet(*args, alg='FISTA', **kwargs):
    ALGS = {'SplitBregman':Wavelet_SplitBregman, 'FISTA':Wavelet_FISTA}
    if alg in ALGS:
        return ALGS[alg](*args, **kwargs)
    else:
        raise NotImplementedError
...
# x = np.random.rand(100)
# b = np.random.rand(200); b = A.dot(x)
# rec = Wavelet((100,), wavelet='None').run(op=A, data=b, maxiter=100, weight=0,
#                           callback=('primal', 'gap', 'step'))[0]
# print('\nFinal error:', abs(A.dot(rec) - b).max(), abs(A.T.dot(A.dot(rec) - b)).max(), '\n')
# # y = CG_LSQRS(lambda X:A2.dot(X), A.T.dot(b), x, tol=1e-10)
# # print('\nFinal error:', abs(A.dot(y) - b).max())
# exit()
...
if __name__ == '__main__':
    ...
    WAVE = {'wavelet':'db4'}
    F = NonNeg_prox(data2d.size, WaveletL1(vol_shape=data2d.shape, **WAVE), scale=a)
    F.setprox(b)
    rec1 = F.prox(data2d).reshape(data2d.shape)
    G = Wavelet(data2d.shape, **WAVE); G.setParams(op=scalar_mat((data2d.size,) * 2, scale=1), data=data2d)
    rec2 = G.run(weight=a * b / G.dataScale)
    H = SmoothWavelet(data2d.shape, pos=True, **WAVE); H.setParams(op=scalar_mat((data2d.size,) * 2, scale=1), data=data2d, eps=1e-3)
    rec3 = H.run(weight=a * b / H.dataScale)
    plt.subplot(131); plt.imshow(rec1)
    plt.subplot(132); plt.imshow(rec2)
    plt.subplot(133); plt.imshow(rec3)
    print((G.f(rec1 / G.dataScale) + G.g(rec1 / G.dataScale)) / G._prim)
    print((G.f(rec3 / G.dataScale) + G.g(rec3 / G.dataScale)) / G._prim)

    plt.show()
    exit()

    F = TV_prox(data2d.size, diff(data2d.shape, order=1), scale=a, maxiter=1000, tol=1e-5)
    F.setprox(1)
    G = PDHG(A=F.D, op=F.D,
            f=L2(scale=1, translation=data2d),
            g=L1_2(data2d.size, scale=a).dual
        )
#     H = PDHG(A=F.D, op=F.D,
#             f=L2(scale=1, translation=data2d),
#             g=L1_2(data2d.size, scale=a).dual
#         )
    H = TV_SplitBregman(data2d.shape); H.setParams(op=scalar_mat((data2d.size,) * 2, scale=1), data=data2d, weight=a,
                                                   steps='fast')
#     H = TV_FISTA(data2d.shape); H.setParams(op=scalar_mat((data2d.size,) * 2, scale=1), data=data2d, weight=a)
#     H = TV(data2d.shape); H.setParams(op=scalar_mat((data2d.size,) * 2, scale=1), data=data2d, weight=a)
#     H = SmoothTV(data2d.shape); H.setParams(op=scalar_mat((data2d.size,) * 2, scale=1), data=data2d, weight=a, eps=1e-2)
    H = SmoothWavelet(data2d.shape); H.setParams(op=scalar_mat((data2d.size,) * 2, scale=1), data=data2d, weight=a, eps=1e-3)

    rec = [None] * 3
#     rec[0] = H.run(balance=1, maxiter=100, callback_freq=10, steps='fast',
#                           callback=('primal', 'step', 'gap'))[0].reshape(data2d.shape)
    rec[0] = H.run(balance=1, maxiter=100, callback_freq=10,
                          callback=('primal', 'step'))[0].reshape(data2d.shape)
    rec[1] = G.run(balance=1, maxiter=100, callback_freq=100, steps='adaptive',
                        callback=('primal', 'step', 'gap'))[0].reshape(data2d.shape)
    rec[2] = F.prox(data2d).reshape(data2d.shape)

    print('%.2f\t' * 3 % tuple(.5 * ((r - data2d) ** 2).sum() + F(r) for r in rec))

    plt.subplot(221); plt.imshow(data2d)
    plt.subplot(222); plt.imshow(rec[0]); plt.title('TV code')
    plt.subplot(223); plt.imshow(rec[1]); plt.title('PDHG')
    plt.subplot(224); plt.imshow(rec[2]); plt.title('FISTA')
    plt.show()
    exit()

    data3d = binary_blobs(length=128, n_dim=3,
                        volume_fraction=0.01, seed=1).astype('f4')
    angles = np.linspace(0, np.pi, 20)  # 180
    # dim=0 is tilt axis
    # dim=1 is y/z
...
#     rec3d = sart.run(data=sino3d, op=Xray3, min_val=0)
    ...
    # TV reconstruction
    rec2d = TV(vol2d, alg='PDHG').run(data=sino2d, maxiter=200, weight=0.01,
...
#     rec2d = Wavelet(vol2d, wavelet='db4', alg='FISTA').run(data=sino2d, maxiter=100, weight=1e-4,
#                           callback=('primal', 'gap', 'step'))[0]
